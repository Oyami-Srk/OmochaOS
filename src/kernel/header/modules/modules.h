#ifndef __MODULES_H__
#define __MODULES_H__

// code below is auto-generated by python script
// {Insert Below}

// System Task's process
extern void SysTask(void);
// VFS Process
extern void Task_VFS(void);
_Noreturn // TTY process
    extern void
    Task_TTY(void);
_Noreturn // Memory Process
    extern void
    Task_Memory(void);
_Noreturn // HD Process
    extern void
    Task_HD(void);
// FS Process
extern void Task_FS(void);

#define __MODULES_COUNT__ 6
#define __MODULES_ENTRIES__                                                    \
    {                                                                          \
        (unsigned int)SysTask, (unsigned int)Task_VFS, (unsigned int)Task_TTY, \
            (unsigned int)Task_Memory, (unsigned int)Task_HD,                  \
            (unsigned int)Task_FS                                              \
    }
#define __MODULES_PREFERRED_PID__                                              \
    {                                                                          \
        (unsigned int)2, (unsigned int)0xFFFFFFFE, (unsigned int)0xFFFFFFFE,   \
            (unsigned int)0xFFFFFFFE, (unsigned int)0xFFFFFFFE,                \
            (unsigned int)0xFFFFFFFE                                           \
    }

// {Insert Above}
// code above is auto-generated by python script

#endif // __MODULES_H__
